 ...
 * 
 * This implementation uses a multi-producer, multi-consumer pattern
    ...
    fun offer(record: DLQRecord): Boolean {
        while (true) {
            ...
            if (tail.compareAndSet(currentTail, currentTail + 1)) {
                ...
                // Wait for any previous writer to finish if needed
    ...
    fun poll(): DLQRecord? {
        while (true) {
            ...
            if (head.compareAndSet(currentHead, currentHead + 1)) {
                ...
                // Wait for the writer to finish if needed
                var record: DLQRecord?
                while (true) {
                    record = buffer.get(index)
                    if (record != null) {
                        break
                    }
                    Thread.yield()
